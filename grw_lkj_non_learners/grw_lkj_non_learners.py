...
os.chdir("/grw_lkj_non_learners/")

amps = np.load("/grw_lkj_non_learners/data/non_leaerners_200ms_baseline.npy")

times = np.load("/grw_lkj_non_learners/data/times_200ms_baseline.npy")
...
with pm.Model() as mod:
    ...
    σ = [pm.HalfNormal('σ'+str(c), 1.0, shape=S) for c in range(C)]
...
with mod:
    ppc = pm.sample_prior_predictive(samples=2000, var_names=["y0"])

ppc = ppc['y0']
hdis = az.hdi(ppc[:,12,:], hdi_prob=0.9)
plt.plot(times, amps[0,12,:], color='grey', linestyle='-', label='Learners: observed mean')
plt.plot(times, ppc[:,12,:].mean(axis=0), color='m', linestyle='-', label='Learners: predicted mean')
...
plt.savefig('non_learners_prior_pred_pz.png', dpi=300)
plt.close()


# with mod:
#     trace = pm.sample(2000, tune=2000, chains=4, cores=8, init='adapt_diag', target_accept=0.95)

    
tracedir = "/grw_lkj_non_learners/trace/"
...
chans = pd.read_csv("/grw_lkj_non_learners/data/chans.csv")['chans'].values
path = "/grw_lkj_non_learners/electrodes_contrasts/"
for e in tqdm(range(E)): 
    ...
    axs[1,1].axis("off")
    plt.tight_layout()
    plt.savefig(path+chan+'_posteriors_non_learners.png', dpi=300)
    plt.close()
...
info_path = "/grw_lkj_non_learners/data/info_200ms_baseline.pickle"
...
plt.tight_layout()
plt.savefig('non_learners_tones_correlations_pz.png', dpi=300)
plt.close()
...
######### Save summaries ##########
#summpath = "/grw_lkj_non_learners/tranks/"
...
path = "/grw_lkj_non_learners/tranks/"
